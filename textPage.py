...
def plotPie(labels, values):
    fig = go.Figure(go.Pie(labels=labels, values=values, hoverinfo="label+percent", textinfo="value"))
    st.plotly_chart(fig)
...
def renderPage():
    ...
    st.text("Analyzing text data given by the user and finding sentiments within it.")
    userText = st.text_input('User Input', placeholder='Input text HERE')
    type = st.selectbox('Type of analysis', ('Positive/Negative/Neutral - TextBlob', 'Happy/Sad/Angry/Fear/Surprise - text2emotion'))
